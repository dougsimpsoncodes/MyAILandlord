...
export function useProfileSync() {
  ...
  const { isSignedIn, isLoading, user, processingInvite } = useAppAuth()
    ...
    async function run() {
      try {
        ...
        if (pendingInvite && !profile) {
          log.info('useProfileSync: Skipping profile creation - pending invite detected in AsyncStorage', {
        ...
        if (profileLoading) {
          log.info('useProfileSync: ProfileContext loading, waiting...')
          return
        }
        ...
        if (profile === undefined) {
          log.info('useProfileSync: ProfileContext not ready yet (profile still undefined)')
          return
        }

        log.info('useProfileSync: ProfileContext ready, profile state:', {
        ...
        if (profile?.role) {
          ...
          log.info('useProfileSync: Using database role (source of truth):', dbRole)
        ...
        log.info('useProfileSync: role determination for new profile:', {
        ...
        if (profile === null && !refreshedOnceRef.current) {
          ...
          log.info('useProfileSync: profile null, refreshing once before creation')
        ...
        if (profile === null) {
          // Create new profile
          log.info('useProfileSync: Creating new profile with role:', finalRole)
          ...
          log.info('useProfileSync: Updating profile role to:', finalRole)
