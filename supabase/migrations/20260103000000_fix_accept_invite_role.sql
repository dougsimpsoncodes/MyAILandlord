-- Fix accept_invite RPC to always set tenant role (not COALESCE)
-- This matches the fix applied to signup_and_accept_invite
-- Both RPCs now properly convert users to tenants when accepting invites
...
BEGIN
  ...
  IF v_already_linked THEN
    -- FIXED: Still update role to tenant even if already linked
    UPDATE public.profiles SET role = 'tenant' WHERE id = v_user_id;
  ...
  -- FIXED: Always set role to tenant (no COALESCE)
  UPDATE public.profiles SET role = 'tenant' WHERE id = v_user_id;
...
Fixed: Always sets role to tenant, even if profile already exists with different role.
This prevents race condition where useProfileSync creates profile with default landlord role.';
