...
async function provisionLandlordWithProperty(env: RequiredEnv): Promise<LandlordScenario> {
  ...
  const propertyName = `RW QA Property ${runId}`;
  const propertyAddress = '100 QA Lane, Los Angeles, CA 90001';
...
test.describe('Real-world Manual-parity Invite Flow', () => {
  test('click/type/link path: landlord send invite button to tenant accepted property', async ({ browser, page }, testInfo) => {
    test.slow();
    ...
    try {
      ...
      await expect(tenantPage.getByText(/QA Lane|Los Angeles/i)).toBeVisible({ timeout: 20_000 });
